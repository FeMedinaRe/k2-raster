...
void run_queries(std::string k2raster_filename, const std::string &query_filename, bool set_check) {

    /*********************/
    /* Read queries      */
    /*********************/
    std::vector<k2raster::query<false, true>> queries;

    std::ifstream query_file(query_filename);
    assert(query_file.is_open() && query_file.good());

    while (query_file.good()) {
        k2raster::query<false, true> query(query_file, false);
        if (query_file.good()) {
    ...
    for (uint r = 0; r < NREPS; r++) {
        ...
        for (const auto &query : queries) {
            ...
            size_t n_cells = k2_raster.get_cells_by_value(query.xini, query.xend, query.yini, query.yend,
                                                          query.valini, query.valend, result);
            ...
            if (set_check) {
                ...
                for (const auto &cell : result) {
                    value = k2_raster.get_cell(cell.first, cell.second);
                    if (value < query.valini || value > query.valend) {
                        std::cout << "[ERROR] Cell (" << cell.first << ", " << cell.second << ")";
                        std::cout << " is not within range [ " << query.valini << " - " << query.valend << "]"
                                  << std::endl;
                        exit(-1);
                    }
                }
                ...
                for (auto x = query.xini; x <= query.xend; x++) {
                    for (auto y = query.yini; y <= query.yend; y++) {
                            ...
                            if (value >= query.valini && value <= query.valend) {
                                ...
                                if (!found) {
                                    std::cout << "[ERROR] Cell (" << x << ", " << y << ") not found" << std::endl;
                                    exit(-1);
                                }
#endif
                            }
