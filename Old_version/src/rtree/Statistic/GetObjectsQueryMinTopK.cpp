...
#include <rtree/Statistic/QueryStrategy/GetObjectsQueryMinTopK.h>
...
namespace k2raster_rtree_static {
    ...
    int GetObjectsQueryMinTopK::getMBRValue(BasicSubMatrix *basicSubMatrix) {
        return basicSubMatrix->minValue;
    }

    int GetObjectsQueryMinTopK::getMBRLeafValue(uint xini, uint xend, uint yini, uint yend,
                                                BasicSubMatrix *basicSubMatrix) {
        return this->raster->getMinValueWindow(xini, xend, yini, yend, basicSubMatrix);
    }

    ulong GetObjectsQueryMinTopK::getCells(priorityPointerMBR *ptr) {
