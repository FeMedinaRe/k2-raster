...
#include <rtree/Statistic/QueryStrategy/GetObjectsQueryMaxTopK_old.h>
...
namespace k2raster_rtree_static {
    ...
    /****** Process MBR ******/
    void GetObjectsQueryMaxTopKOLD::getNextEntry(const IEntry &entry, id_type &nextEntry, bool &hasNext) {
        ...
        if (n != 0) {
            ...
            for (uint32_t cChild = 0; cChild < n->getChildrenCount(); cChild++) {
                ...
                if (n->isLeaf()) {
                    ...
                    childPointer->isRealValue = true;               // It is the real max value of the MBR
                    childPointer->value = value;
                } else {
                    childPointer->isRealValue = false;              // pointer.maxValue is the possible max value of the MBR
                    childPointer->value = minSubMatrix->maxValue;
                } // END IF isLeaf
    ...
    void GetObjectsQueryMaxTopKOLD::setNextEntry(id_type &nextEntry, bool &hasNext) {
        ...
        while (!this->pointers.empty()) {
            ...
            if (this->pointers.top()->isRealValue) {
                ...
                // Add Result              //
                // ----------------------- //
                this->maxValues[this->numResults] = ptr->value;
                ...
                if (this->withPositions) {
                    ...
                    ulong  num2 = this->raster->getCellsByValue(ptr->positions->xini, ptr->positions->xend, ptr->positions->yini, ptr->positions->yend,
                                                  ptr->value, ptr->value, ptr->pointer, this->positions[this->numResults], false);

                    if (num2 == 0) {
                        printf("Error getting position for MBR %lu and maxValue %i\n", ptr->id, ptr->value);
                        exit(-1);
                    }
    ...
    //******************* SETTERS AND GETTERS ******************************//
    //**********************************************************************//
    void GetObjectsQueryMaxTopKOLD::initialize() {
        GetObjectsQueryBasic::initialize();
        this->nextPointer = nullptr;
        this->numResults = 0;
        this->maxValues = new int[this->k];
        this->MBRIds = new id_type[this->k];

        if (this->withPositions) {
            this->positions = new uint*[this->k];
            this->numOfPositions = new uint[this->k];
        } else {
    ...
    void GetObjectsQueryMaxTopKOLD::clear() {
