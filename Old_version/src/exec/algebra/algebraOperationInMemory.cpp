...
int main(int  argc, char ** argv) {
    ...
    // Obtain basic params and malloc memory
    const uint nrows = std::min(raster1->getDimSize(1), raster2->getDimSize(1));
    const uint ncols = std::min(raster1->getDimSize(2), raster2->getDimSize(2));
    ...
    // Decompress data
    int *data1 = raster1->decompress();
    uint sizeY1 = raster1->getDimSize(2);
    if (!check) {
        delete (raster1);
    }

    int *data2 = raster2->decompress();
    uint sizeY2 = raster2->getDimSize(2);
    if (!check) {
        delete (raster2);
    }
    int val1, val2;
    for (uint x = 0; x < nrows; x++) {
        for (uint y = 0; y < ncols; y++) {
            val1 = data1[x * sizeY1 + y];
            val2 = data2[x * sizeY2 + y];
            switch (op) {
                case k2raster_static::OperationRaster::OPERATION_SUM:
                    newData[x][y] = val1 + val2;
                    break;
                case k2raster_static::OperationRaster::OPERATION_SUBT:
                    newData[x][y] = val1 - val2;
                    break;
                case k2raster_static::OperationRaster::OPERATION_MULT:
                    newData[x][y] = val1 * val2;
                    break;
                default:
    ...
    k2raster_static::K2Raster *rasterResult = new k2raster_static::K2Raster(nrows, ncols, newData, k1, k2, levelK1);
