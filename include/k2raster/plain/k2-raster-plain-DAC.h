...
#ifndef K2RASTER_PLAIN_DAC_H_
#define K2RASTER_PLAIN_DAC_H_

#include <k2raster/plain/k2-raster-plain.h>

namespace k2raster_static {

class K2RasterPlainDAC : public K2RasterPlain {
  public:
    /****** Constructors ******/
    K2RasterPlainDAC(uint sizeX, uint size, int **data, uint K1, uint K2, uint levelK1, uint plainLevels);
    ~K2RasterPlainDAC();
    ...
    static K2RasterPlainDAC *load(std::ifstream &in);
  ...
  protected:
    ...
    // Attributes
    cds_static::DirectAccess *lastValues;

};
}


#endif // K2RASTER_PLAIN2_H_
