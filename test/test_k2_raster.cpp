...
// TEST CLASS
template<class T>
class test_k2_raster : public ::testing::Test { };
...
TYPED_TEST_CASE(test_k2_raster, Implementations);
...
TYPED_TEST(test_k2_raster, CretateInMemory){
    ...
    TypeParam k2raster(values,rows, cols, k1, k2, level_k1);
...
TYPED_TEST(test_k2_raster, LoadFromFile){
    TypeParam k2raster;
...
TYPED_TEST(test_k2_raster, QueryGet){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster, QueryGetCellsByValue){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster, QueryGetValuesWindow){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster, QueryCheckValuesWindowStrong){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster, QueryCheckValuesWindowWeak){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
