/*  
 * Created by Fernando Silva on 8/04/20.
 *
...
// TEST CLASS
template<class T>
class test_k2_raster_temporal_h : public ::testing::Test { };
...
TYPED_TEST_CASE(test_k2_raster_temporal_h, Implementations);

// TEST Encode - Create k2_raster temporal (in-memory)
TYPED_TEST(test_k2_raster_temporal_h, CreateInMemory){
...
TYPED_TEST(test_k2_raster_temporal_h, LoadFromFile){
    // Load structure
    TypeParam k2raster;
...
TYPED_TEST(test_k2_raster_temporal_h, QueryGet){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster_temporal_h, QueryGetCellsByValue){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster_temporal_h, QueryGetValuesWindow){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster_temporal_h, QueryCheckValuesWindowStrong){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster_temporal_h, QueryCheckValuesWindowWeak){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
