...
// TEST CLASS
template<class T>
class test_k2_raster_heuristic : public ::testing::Test { };
...
TYPED_TEST_CASE(test_k2_raster_heuristic, Implementations);


// TEST Encode - Create k2_raster_heuristic
TYPED_TEST(test_k2_raster_heuristic, CretateAndStored){
...
TYPED_TEST(test_k2_raster_heuristic, CreateInMemoryNotPlainLevels){

    // Run k2-raster
    TypeParam k2raster(values,rows, cols, k1, k2, level_k1, 0);
...
TYPED_TEST(test_k2_raster_heuristic, LoadFromFile){
    TypeParam k2raster;
...
TYPED_TEST(test_k2_raster_heuristic, QueryGet){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster_heuristic, QueryGetCellsByValue){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
// TEST Query - Random Get_Cells_by_Value
TYPED_TEST(test_k2_raster_heuristic, QueryGetValuesWindow){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster_heuristic, QueryCheckValuesWindowStrong){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
...
TYPED_TEST(test_k2_raster_heuristic, QueryCheckValuesWindowWeak){
    TypeParam k2raster;
    std::ifstream output_file(output_filename);
    k2raster.load(output_file);
    output_file.close();
