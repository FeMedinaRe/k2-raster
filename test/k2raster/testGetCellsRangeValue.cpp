 ...
 *
 * Test: Execute a set of queries of getCellsRangeValue over a k2-raster
 *
...
int main(int argc, char **argv) {
    ...
    for (uint type = K2RASTER; type <= K2RASTER_VOC_ENTROPY; type++) {
        ...
        if (!raster->checkEncode(data, nrows, ncols)) {
            printf("%s getCellsRangeValue (encoding phase): FAIL\n", typeName.data());
            delete (raster);
            continue;
        } else {
            printf("%s getCellsRangeValue (encoding phase): OK\n", typeName.data());
        }
        ...
        for (uint q = 0; q < numOfQueries; q++) {
            uint *positions = (uint *) malloc(((queries[q]->xend - queries[q]->xini + 1) *
                                               (queries[q]->yend - queries[q]->yini + 1)) * sizeof(uint));
            ulong totalCellsQuery = raster->getCellsByValue(*queries[q], positions);
            ulong totalCellsTest = 0;
            for (ulong r = queries[q]->xini; r <= queries[q]->xend; r++) {
                for (ulong c = queries[q]->yini; c <= queries[q]->yend; c++) {
                    uint value = data[r][c];
                    uint posTest = r * ncols + c;
                    if (value >= queries[q]->valini && value < queries[q]->valend) {
                        totalCellsTest++;
            ...
            if (!errorFound && totalCellsTest != totalCellsQuery) {
                printf("Get %lu cells, expected %lu\n", totalCellsQuery, totalCellsTest);
                errorFound = true;
            }

            if (errorFound) {
                queries[q]->printQuery(q + 1);
                printf("\n");
                break;
            }
        }
        if (errorFound) {
            printf("%s getCellsRangeValue (random queries): FAIL\n", typeName.data());
            delete (raster);
            continue;
        } else {
            printf("%s getCellsRangeValue (random queries): OK\n", typeName.data());
        }

        printf("%s getCellsRangeValue: OK\n", typeName.data());
        delete (raster);
    } // END FOR type
